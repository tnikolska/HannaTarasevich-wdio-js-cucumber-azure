...
let reportFileName = fs.readdirSync('./reports/cucumberJS');
...
const parseJsonReport = async function() {
  ...
  for (let elem of report[0].elements) {
    let scenarioStatus;
    let testCaseId;
    let screenshot;
    let errorMessage;
...
const getRunID = async function() {
  ...
  let runId = response.data.value.filter(item => item.name === azureConfig.runName);
...
const updateRunResultsWithErrors = async function() {
  ...
  let runResults = await parseJsonReport();
  let azureTestRunResult = (await getRunTestResults()).data.value;

  let requestBody = [];

  for(let item of azureTestRunResult){
...
const updateTestSuiteWithResults = async function() {
    ...
    for (let item of items) {
      let result = runResults.filter(result => result.id == item.testCaseReference.id);
